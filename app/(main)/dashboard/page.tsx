...
function DashboardContent() {
  ...
  const totalSeconds = enrollments.reduce((acc, e) => acc + (e.progress?.watchedSeconds || 0), 0);
      ...
      <div>
        ...
        {enrollments.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            ...
            <Link href="/courses" className="inline-block bg-gradient-to-r from-teal-500 to-blue-500 hover:from-teal-600 hover:to-blue-600 text-white font-semibold py-2 px-6 rounded-lg transition">
              강좌 탐색하기
            </Link>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    {item.progress?.completed ? (
                      ...
                      <span className="text-teal-600 text-xs">
                        {Math.floor((item.progress?.watchedSeconds || 0) / 60)}분 학습
                      </span>
                    )}
                  </div>
                </div>
