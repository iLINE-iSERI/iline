...
function NewNoticeContent() {
  ...
  const handleAddUrl = () => {
    ...
    if (attachmentUrls.length >= MAX_ATTACHMENTS) {
      alert(`최대 ${MAX_ATTACHMENTS}개까지 가능합니다`);
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createPost({
        ...
        // 호환: 첫 번째 이미지를 옛 단일 필드에도 (목록 썸네일에서 사용)
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            최대 {MAX_ATTACHMENTS}개 · 각 5MB 이하 · jpg/png/webp 등 이미지 · 파일 선택 시 여러 파일 한 번에 선택 가능
          </p>
                ...
                <div key={url + idx} className="relative group bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
                  ...
                  <div className="absolute inset-x-0 bottom-0 flex gap-0.5 p-1 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition">
                    <button
                      ...
                      disabled={idx === 0}
                      className="text-white text-xs px-1 hover:text-purple-200 disabled:text-gray-400"
                      title="앞으로"
                    >
                      ◀
                    </button>
                    <button
                      type="button"
                      onClick={() => moveAttachment(idx, 1)}
                      disabled={idx === attachmentUrls.length - 1}
                      className="text-white text-xs px-1 hover:text-purple-200 disabled:text-gray-400"
                      title="뒤로"
                    >
                      ▶
