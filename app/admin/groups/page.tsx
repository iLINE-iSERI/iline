    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <div className="flex gap-3">
          <input
            ...
            placeholder="그룹 이름 (예: 초등학생)"
            className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition"
          />
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          {savingOrder && <span className="text-xs text-purple-600 animate-pulse">순서 저장 중...</span>}
        </div>
        {groups.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-400">등록된 그룹이 없습니다</p>
            <p className="text-gray-400 text-sm mt-1">위에서 그룹을 추가하거나 &ldquo;기본 그룹 일괄 등록&rdquo;을 클릭하세요</p>
          </div>
        ) : (
          <ul>
            {groups.map((g, idx) => {
              ...
              return (
                <li
                  key={g.id}
                  className={`flex items-center justify-between px-6 py-3 ${idx < groups.length - 1 ? 'border-b border-gray-50' : ''} ${isDup ? 'bg-red-50' : ''}`}
                >
                  {editId === g.id ? (
                    <div className="flex gap-2 flex-1">
                      ...
                      <button onClick={() => { setEditId(null); setEditName(''); }} className="text-gray-400 font-semibold text-sm px-2">취소</button>
                    </div>
                  ) : (
                    <>
                      <div className="flex items-center gap-3 flex-grow">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <button
                            ...
                            className="text-gray-400 hover:text-purple-600 disabled:opacity-30 disabled:hover:text-gray-400 text-xs leading-none"
                            title="위로"
                          >
                            ▲
                          </button>
                        ...
                        <span className="w-7 h-7 bg-purple-100 text-purple-600 rounded-lg flex items-center justify-center text-xs font-bold">
                          {idx + 1}
                        </span>
